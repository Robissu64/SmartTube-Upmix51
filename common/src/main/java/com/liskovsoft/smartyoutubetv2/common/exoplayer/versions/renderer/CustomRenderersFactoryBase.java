...
public abstract class CustomRenderersFactoryBase extends DefaultRenderersFactory {
    private final Context upmixContext;

    public CustomRenderersFactoryBase(Context context) {
        super(context);
        upmixContext = context.getApplicationContext();
    }

    @Override
    protected AudioProcessor[] buildAudioProcessors() {
        return UpmixAudioProcessor.ENABLE_STEREO_TO_51_UPMIX
                ? new AudioProcessor[] {new UpmixAudioProcessor(new Upmix51Capabilities(upmixContext))}
                : super.buildAudioProcessors();
    }
