...
public final class DefaultAudioSink implements AudioSink {
  ...
  private int upmixLoggedRouteId = -1;
  ...
  public void configure(
    ...
    // Decide from the ORIGINAL decoder format, before any channel mapper can downmix it.
    for (AudioProcessor processor : toIntPcmAvailableAudioProcessors) {
      if (processor instanceof UpmixAudioProcessor) {
        ((UpmixAudioProcessor) processor).setSourceAllowed(
            inputChannelCount == 2 && inputEncoding == C.ENCODING_PCM_16BIT
                && outputChannels == null && !tunneling, audioAttributes);
        if (!Util.isEncodingLinearPcm(inputEncoding)) {
          android.util.Log.i(UpmixAudioProcessor.TAG, "Encoded passthrough preserved; upmix bypassed");
        }
      }
    }
    ...
    try {
      ...
      // Capability checks can race with HDMI disconnection or platform resource changes.
      boolean disabledUpmix = false;
      for (AudioProcessor processor : availableAudioProcessors) {
        if (processor instanceof UpmixAudioProcessor && processor.isActive()) {
          ((UpmixAudioProcessor) processor).disableForSinkLifetime();
          disabledUpmix = true;
        }
      }
      if (!disabledUpmix) throw e;
      android.util.Log.w(UpmixAudioProcessor.TAG, "Invalid 5.1 buffer configuration - falling back to stereo", e);
      configure(inputEncoding, inputChannelCount, inputSampleRate, specifiedBufferSize,
          outputChannels, trimStartFrames, trimEndFrames);
      return;
    }
  ...
  private void initialize(long presentationTimeUs) throws InitializationException {
    ...
    if (isUpmixConfiguration()) {
      android.util.Log.i(UpmixAudioProcessor.TAG,
          "Attempting PCM16 5.1 AudioTrack: " + configuration.outputSampleRate
              + "Hz mask=0x" + Integer.toHexString(configuration.outputChannelConfig)
              + " buffer=" + configuration.bufferSize);
    }
    try {
      audioTrack = Assertions.checkNotNull(configuration)
          .buildAudioTrack(tunneling, audioAttributes, audioSessionId);
    } catch (InitializationException | RuntimeException e) {
      if (!isUpmixConfiguration()) throw e;
      android.util.Log.w(UpmixAudioProcessor.TAG,
          "AudioTrack 5.1 initialization failed - falling back to stereo", e);
      configureStereoAfterUpmixFailure();
      audioTrack = configuration.buildAudioTrack(tunneling, audioAttributes, audioSessionId);
    }
    if (isUpmixConfiguration()) {
      android.util.Log.i(UpmixAudioProcessor.TAG, "AudioTrack initialized: PCM16 6ch "
          + configuration.outputSampleRate + "Hz mask=0x"
          + Integer.toHexString(configuration.outputChannelConfig)
          + "; channel config=5.1; buffer=" + configuration.bufferSize
          + "; physical eARC output still requires verification");
    }
    upmixLoggedRouteId = -1;
  ...
  private boolean isUpmixConfiguration() {
    if (configuration == null || !configuration.isInputPcm
        || configuration.inputPcmFrameSize != 4 || configuration.outputPcmFrameSize != 12) return false;
    for (AudioProcessor processor : configuration.availableAudioProcessors) {
      if (processor instanceof UpmixAudioProcessor && processor.isActive()) return true;
    }
    return false;
  }

  // Called before consuming decoder PCM. Keep trimming, sample rate, and timestamp accounting.
  private void configureStereoAfterUpmixFailure() {
    int channels = 2;
    int rate = configuration.inputSampleRate;
    ...
    for (AudioProcessor processor : configuration.availableAudioProcessors) {
      ...
      try {
        processor.configure(rate, channels, encoding);
      } catch (AudioProcessor.UnhandledFormatException e) {
        throw new IllegalStateException("Previously supported stereo format rejected", e);
      }
      if (processor.isActive()) {
        channels = processor.getOutputChannelCount();
        rate = processor.getOutputSampleRateHz();
        encoding = processor.getOutputEncoding();
      }
    }
    configuration = new Configuration(true, 4, configuration.inputSampleRate,
        Util.getPcmFrameSize(encoding, channels), rate, getChannelConfig(channels, true),
        encoding, 0, true, true, configuration.availableAudioProcessors);
    android.util.Log.i(UpmixAudioProcessor.TAG, "Stereo fallback configured; upmix disabled for this sink");
  }
  ...
  public boolean handleBuffer(ByteBuffer buffer, long presentationTimeUs)
    ...
    if (isUpmixConfiguration() && Util.SDK_INT >= 23) {
      android.media.AudioDeviceInfo route = audioTrack.getRoutedDevice();
      if (route != null && route.getId() != upmixLoggedRouteId) {
        upmixLoggedRouteId = route.getId();
        android.util.Log.i(UpmixAudioProcessor.TAG, "Actual AudioTrack route: id=" + route.getId()
            + " type=" + route.getType() + " name=" + route.getProductName()
            + "; track channels=" + audioTrack.getChannelCount());
      }
    }
